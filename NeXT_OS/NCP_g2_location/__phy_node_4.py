...
#------------------------------------------------------
# Code Date: 2023-07-12 13:02:27.230334
# Author: Zhangyu Guan
#######################################################
...
def func(objvar, sign=-1.0):
	utlt = -expr_4_lag*log(1 + lkpwr_link_3/(lkitf_link_3*((objvar[0] - fx_crd_x_link_3)**2 + (objvar[1] - fx_crd_y_link_3)**2)**2))/log(2)
	pnl = - sum(sum(pnl_coefficient, axis = 1)*(np.array([coord_x_link_3, coord_y_link_3]) - np.array([objvar[0], objvar[1]])))
	ovl_utlt = utlt + pnl
	return ovl_utlt
