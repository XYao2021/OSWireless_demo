...
def get_hsubname(sym_expr, obj_xpd):
    ...
    pos_sfx = str_expr.find(dcp_name.suffix)                                     # starting position of suffix, i.e., ___
     ...
     #/*
    # Guan, change,  Dec. 5, 2017
    # Currently, lkpwr and lkcap are grouped into different horizontal subproblems while they belong to the same subproblem.
    # The new code will link these different elements to the same network element, i.e., the link   
    
    #str_keyelmt = str_expr[pos_pfx: pos_sfx + len(dcp_name.suffix) + dcp_name.zflen]    # retrieve the key_elmt
    ...
    # print('---')
    # elmt = obj_xpd.get_netelmt('ssrate')
    # print(elmt.parent.type)
    
    # print(dcp_name.hsub)
    # print(str_expr[(pos_pfx+len(dcp_name.prefix)): pos_sfx])
    # print(str_keyelmt)
    # exit(0)    
...
class hsub(net_func.netelmt_group):
    ...
    def get_optvar(self):
        ...
        # 5/22/2017: Checking not needed here, only the first element will be used any way
        #/*
        # Currently only one optimization variable is considered
        #if len(lst_var) != 1:
        #    print('Error: One optimization variable required, but %d given' %len(lst_var))
        #    exit(0)
        #*/
