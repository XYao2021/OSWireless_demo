...
class fam_xxx:
    ...
    def genlst(self, fam_hldr):
        ...
        '''
        # no need to turn to full list, instead generate the full list directly
        
        # # get the full family set if haven't yet
        # if self.full_list == None:
            # self.full_list = self.get_fulfamset(fam_hldr)   
            # #print('*********', fset.id, fset.lst)           
        #lst = self.full_list.lst
