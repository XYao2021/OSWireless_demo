                            ...
                            EnergyDetectorIIR& detector, std::atomic<bool>& stop_sign)
{
    ...
    const size_t WARMUP_BLOCKS = 10;  // Discard first 10 blocks
    
    std::cout << "[DETECTOR] Discarding " << WARMUP_BLOCKS 
              << " warmup blocks..." << std::endl;
    
    // Throw away first N blocks
    for (size_t i = 0; i < WARMUP_BLOCKS; i++) {
        while (!input_fifo.pop(message) && !stop_sign.load()) {
            std::this_thread::sleep_for(std::chrono::milliseconds(10));
        }
        std::cout << "[DETECTOR] Discarded warmup block " << (i+1) 
                  << "/" << WARMUP_BLOCKS << std::endl;
    }
    
    std::cout << "[DETECTOR] Warmup complete, starting detection..." << std::endl;
