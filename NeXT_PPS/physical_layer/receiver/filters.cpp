                                ...
                                std::atomic<bool>& stop_sign, const std::string role)
{
    ...
    while(!stop_sign.load() || modulation_fifo.size() > 0){
        // Debugging printout
        ...
        // save_block_to_txt(filtered, 0, "filtered");
                        ...
                        std::atomic<bool>& stop_sign, const std::string role)
{
    ...
    rrc_pulse(org_taps.data(), (num_taps-1)/2, U, D, roll_off);
    // save_block_to_txt(taps, 0, "taps");
