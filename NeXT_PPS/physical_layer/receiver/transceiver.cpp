                     ...
                     std::atomic<bool>& stop_sign)
{
    ...
    while (!stop_sign.load() || filtered_fifo.size() > 0){
        ...
        while (samples_sent < samples.size() && !stop_sign.load() && !transmission_failed){
        ...
        // // CRITICAL: Check if there's a gap
        // if (tx_duration > (samples.size() / tx_rate * 1.5)) {
        //     std::cout << "[TX WARNING] ⚠️  Transmission too slow! Possible gaps!" << std::endl;
        // }

        // if (transmission_failed) {
        //     std::cerr << "[USRP TX] Block #" << block_id << " transmission FAILED" << std::endl;
        //     consecutive_errors++;
        //     if (consecutive_errors >= MAX_CONSECUTIVE_ERRORS) {
        //         std::cerr << "[USRP TX] Too many failures, stopping transmission" << std::endl;
        //         break;
        //     }
        // } else {
        //     std::cout << "[USRP TX] Block #" << block_id << " transmitted successfully:" << std::endl;
        //     std::cout << "  Samples sent: " << samples_sent << "/" << samples.size() << std::endl;
        //     std::cout << "  Chunks: " << chunks_sent << std::endl;
        //     std::cout << "  Duration: " << (samples_sent / tx_rate * 1000) << " ms" << std::endl;
    ...
    // // Check async messages for errors
    // uhd::async_metadata_t async_md;
    // size_t async_messages = 0;
    // while (tx_stream->recv_async_msg(async_md, 0.1)) {
    //     async_messages++;
        
    //     if (async_md.event_code == uhd::async_metadata_t::EVENT_CODE_BURST_ACK) {
    //         std::cout << "[USRP TX] Burst ACK received" << std::endl;
    //     } else {
    //         std::cerr << "[USRP TX] Async event code: " << async_md.event_code << std::endl;
    ...
    //                 std::cerr << "  → TIME ERROR detected!" << std::endl;
    //                 break;
    //             case uhd::async_metadata_t::EVENT_CODE_SEQ_ERROR:
    //                 std::cerr << "  → SEQUENCE ERROR detected!" << std::endl;
    //                 break;
    //             default:
    ...
    // if (total_blocks > 0) {
    //     std::cout << "  Avg samples/block: " << (total_transmitted / total_blocks) << std::endl;
    //     std::cout << "  Total TX time: " << (total_transmitted / tx_rate) << " seconds" << std::endl;
    // }
    // std::cout << "  Async messages: " << async_messages << std::endl;
                    ...
                    int num_requested_samples, size_t samps_per_buff, std::atomic<bool>& stop_sign)
{
    ...
    while (!stop_sign.load() && (num_requested_samples > num_total_recv_samps || num_requested_samples == 0)){
                            ...
                            EnergyDetectorIIR& detector, std::atomic<bool>& stop_sign)
{
    ...
    while (!stop_sign.load() || input_fifo.size() > 0){
                ...
                std::atomic<bool>& stop_sign, const std::string& AGC)
{
    ...
    while(!stop_sign.load() || received_fifo.size() > 0){
