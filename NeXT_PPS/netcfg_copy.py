##! /usr/bin/python
"""
										Layer 2 and Layer 4 Frames

# layer 2 (Datalink)	 ________________________________________________________________________________________
#	STRUCTURE			|_______pktno_mac_______________|_______src_serial_usrp_____________|_______dst_serial_usrp_____|
#	BYTES				|			2					|			16					    |			16
#
#	FIELD						WRITE								READ
#	pktno_mac					struct.pack('h', pktno_mac)			(pktno_mac,) = struct.unpack('h', down_packet[0:2])
#	source_serial_usrp		    + src_serial.ljust(16, b'\x00')		mac_packet[2:18].strip(b'\x00').decode()
# 	dst_serial_usrp			    + dst_serial.ljust(16, b'\x00')		mac_packet[18:34].strip(b'\x00').decode()
#
#
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#	STRUCTURE			|_______pktno_l4________________|_______src_serial_l4_______________|_______dst_serial_l4_______|_____timestamp_____|
#	BYTES				|			8					|			16					   |			16			   |			8
#
#	FIELD						WRITE								READ
#	pktno_l4					struct.pack('l', pktno_l4)			(l4_ack_pktno,) = struct.unpack('l', ack_l4[:8])
#	source_serial_l4			+ src_serial.ljust(16, b'\x00')		l4_packet_sender = ack_l4[8:24].strip(b'\x00').decode()
# 	dst_serial_l4				+ dst_serial.ljust(16, b'\x00')		l4_packet_receiver = ack_l4[24:40].strip(b'\x00').decode()
#	timestamp 					+ struct.pack('d', timestamp)		(timestamp,) = struct.unpack('d', ack_l4[40:48])
"""
...
"IP and Serial Configuration"
# IP of PCs
# IP_PC1 = '10.143.12.110'  # Transmitter PC IP address
# IP_PC2 = '10.245.86.34'  # Receiver PC IP address

IP_PC1 = '10.245.80.237'  # Transmitter PC IP address
IP_PC2 = '10.245.86.34'  # Receiver PC IP address

# IP_PC1 = '0.0.0.0'  # Transmitter PC IP address
# IP_PC2 = '10.245.86.34'  # Receiver PC IP address
...
# B210 USRP serial numbers
serial_usrp_src1 = '30CD424'
serial_usrp_dst1 = '30CD3F7'

# For backward compatibility with existing code
ip_usrp_src1 = serial_usrp_src1
ip_usrp_dst1 = serial_usrp_dst1
...
usrp_serial_2_ndidx = {serial_usrp_src1: 0, serial_usrp_dst1: 1}

# For backward compatibility
usrp_ip_2_ndidx = {ip_usrp_src1: 0, ip_usrp_dst1: 1}

# Serial numbers of sender-type nodes of a session
usrp_serial_sender_of_session = {serial_usrp_src1: [serial_usrp_src1]}

# For backward compatibility
usrp_ip_sender_of_session = {ip_usrp_src1: [ip_usrp_src1]}

# All USRP serials
all_usrp_serial = [serial_usrp_src1, serial_usrp_dst1]

# For backward compatibility
all_usrp_ip = all_usrp_serial

# Arguments for UHD (using serial)
args = ['serial=' + serial_usrp_src1, 'serial=' + serial_usrp_dst1]

# USRP serials list
usrp_serial = [serial_usrp_src1, serial_usrp_dst1]

# For backward compatibility
ip_usrp = usrp_serial

# PC address list
ip_pc = [ip_pc_src1, ip_pc_dst1]

# Previous hop serial
prev_hop_serial = [None, serial_usrp_src1]

# For backward compatibility
prev_hop_usrp = prev_hop_serial

# Previous hop PC
prev_hop_pc = [None, ip_pc_src1]

# Next hop serial
next_hop_serial = [serial_usrp_dst1, None]

# For backward compatibility
next_hop_usrp = next_hop_serial

# Next hop PC
next_hop_pc = [ip_pc_dst1, None]

# Source USRP serial
source_usrp_serial = [None, serial_usrp_src1]

# For backward compatibility
source_usrp = source_usrp_serial

# Source PC
source_pc = [None, ip_pc_src1]

# Destination USRP serial
dest_usrp_serial = [serial_usrp_dst1, None]

dest_usrp = dest_usrp_serial

# Destination PC
dest_pc = [ip_pc_dst1, None]

# Source USRP serial of sessions
source_usrp_session_serial = [serial_usrp_src1, serial_usrp_src1]

source_usrp_session = source_usrp_session_serial

# New mapping dictionaries needed for B210
# Map USRP serial to PC address
serial_to_addr = {serial_usrp_src1: ip_pc_src1, serial_usrp_dst1: ip_pc_dst1}

# Map USRP serial to port
serial_to_port = {serial_usrp_src1: 9000, serial_usrp_dst1: 9001}

# UDP port numbers for listening
udp_port_listen = [9000, 9001]

# UDP port numbers for sending
...
"""
	...
	Elements of each vector represent the interferer
...
itf_relation = np.matrix([itf_src1, itf_dst1])

"Protocol Configuration"
# number of frames at mac layer building one layer-4 packet
number_of_blocks = 1  # number of 255 Byte blocks composing a l2 packet
number_of_frames = 2  # number of layer 2 frames in one layer 4 packet
l4_packets_to_send = 10000  # Total number of layer-4 packets to be sent
...
# l2_size_block = 2048  # fixed, in Byte, maximum 4096/2 = 2048
...
# Updated for B210 serial addressing (13-byte IP -> 13-byte serial)
l4_header_length = 42  # Was 42: 8 bytes pktno + 13 bytes src + 13 bytes dst + 8 bytes timestamp
...
#
# # # Get current and parent directory; parent directory is the overall directory of the whole project
# # c = os.getcwd()									# Current directory
# # p = os.path.dirname(c)      					# Parent directory
#
...
# #################################################################################
# import ptcl_name					# ptcl_name is in NeXT-OSdirectory
# import alglib_config
# import net_name_g2 as net_name		# net_name_g2.py is in NeXT-OS --> wos-network directory
#
# # import ptcl_name, net_name, alglib_config
#
#
# ####################################################################################
# #                         Constants and node information
...
# ip_pc_dst1 		= 	IP_PC1
#
# # this usrp ip is used for UDP connection between the PC and usrp device,
# # and the other usrp device is used for receiving UDP packets from the PC,
# # this is not the network layer usrp ip address. For B210 usrp device, this ip address
# # will be replaced by the serial number.
#
# ip_usrp_src1 	= 	'30CD424'
# ip_usrp_dst1 	= 	'30CD3F7'
#
# # replace the ip_usrp_src1 with the serial number
# # replace the ip_usrp_dst1 with the serial number as well.
#
# # serial_usrp_src1	= ''
# # serial_usrp_dst1	= ''
#
...
# location={}
#
# location[ip_usrp_src1] 	    = 	{'x':0, 'y':0, 'z':0}
# location[ip_usrp_dst1] 	    = 	{'x':3, 'y':0, 'z':0}
#
# # location[ip_usrp_src1] 	    = 	{'x':0, 'y':0, 'z':0}
# # location[ip_usrp_rly11] 		= 	{'x':1, 'y':0, 'z':0}
# # location[ip_usrp_rly12] 		= 	{'x':2, 'y':0, 'z':0}
# # location[ip_usrp_dst1] 	    = 	{'x':3, 'y':0, 'z':0}
# # location[ip_usrp_src2] 	    = 	{'x':0, 'y':3, 'z':0}
# # location[ip_usrp_rly21] 		= 	{'x':1, 'y':3, 'z':0}
# # location[ip_usrp_rly22] 		= 	{'x':2, 'y':3, 'z':0}
# # location[ip_usrp_dst2] 	    = 	{'x':3, 'y':3, 'z':0}
#
#
# coord_list = ['x', 'y']
# ########################################################
...
# # usrp_ip_2_ndidx[ip_usrp_rly11] 	= 	2
# # usrp_ip_2_ndidx[ip_usrp_rly21] 	= 	3
# # usrp_ip_2_ndidx[ip_usrp_rly12] 	= 	4
# # usrp_ip_2_ndidx[ip_usrp_rly22] 	= 	5
# # usrp_ip_2_ndidx[ip_usrp_dst1] 	= 	6
# # usrp_ip_2_ndidx[ip_usrp_dst2] 	= 	7
#
...
# # usrp_ip_sender_of_session[ip_usrp_src2] = [ip_usrp_src2, ip_usrp_rly21, ip_usrp_rly22]
#
# usrp_ip_sender_of_session[ip_usrp_src1] = [ip_usrp_src1]
#
#
# # All usrp IPs
# all_usrp_ip = [	ip_usrp_src1, 		ip_usrp_dst1]
#
# # usrp address
...
#
# # args = [ 'addr='+ip_usrp_src1, 	'addr='+ip_usrp_dst1 ]
#
# args = [ 'serial='+ip_usrp_src1,  'serial='+ip_usrp_dst1 ]
#
# #serial number for args
#
# # ip_usrp
# #   src1   			src2  			src3
# #	rly11   		rly21   		rly31
...
#
# ip_usrp = [ ip_usrp_src1,   	ip_usrp_dst1 ]
#
#
# # ip_pc
# #   src1   			src2
...
# # 				 ip_usrp_rly12,     ip_usrp_rly22]
#
# prev_hop_usrp = [ None,             ip_usrp_src1 ]
#
# # prev_hop_pc
# #   src1   			src2
# #	rly11   		rly21
...
# # 				None,              None]
#
# next_hop_usrp = [ ip_usrp_dst1,   	None ]
#
# # next_hop_pc
# #   src1   			src2
# #	rly11   		rly21
...
# # 						  ip_usrp_src1,   		ip_usrp_src2]
#
# source_usrp_session   =  [ ip_usrp_src1,   	    ip_usrp_src1 ]
#
...
#
#
# # these UDP port numbers are changed to USB port number for B210
#
# udp_port_send =   [	8000,  	8001]
#
# # udp_port_send =   [	8000,  	8001,\
...
# port_usrp_dic = {}
#
# port_usrp_dic[ip_usrp_src1] = udp_port_listen[0]	# 1 session
# port_usrp_dic[ip_usrp_dst1] = udp_port_listen[1]
#
# # port_usrp_dic[ip_usrp_src1] = udp_port_listen[0]	# Session 1
# # port_usrp_dic[ip_usrp_rly11] = udp_port_listen[2]
# # port_usrp_dic[ip_usrp_rly12] = udp_port_listen[4]
# # port_usrp_dic[ip_usrp_dst1] = udp_port_listen[6]
# #
# # port_usrp_dic[ip_usrp_src2] = udp_port_listen[1]	# Session 2
# # port_usrp_dic[ip_usrp_rly21] = udp_port_listen[3]
# # port_usrp_dic[ip_usrp_rly22] = udp_port_listen[5]
# # port_usrp_dic[ip_usrp_dst2] = udp_port_listen[7]
#
#
...
# neighbors_dic = {}
#
# # only 1 session : src and dest are neigbor, rly has src and dst, dst has rly as neigbor. the definition of neighbor can be changed
# neighbors_dic[ip_usrp_src1] = [ip_usrp_dst1]
# neighbors_dic[ip_usrp_dst1] = [ip_usrp_src1]
#
# # #session 1 : src has rly as neigbor, rly has src and dst, dst has rly as neigbor. the definition of neighbor can be changed
...
# # neighbors_dic[ip_usrp_rly12] = [ip_usrp_rly11,ip_usrp_dst1]
# #
# # #session 2
...
# # neighbors_dic[ip_usrp_rly22] = [ip_usrp_rly21,ip_usrp_dst2]
#
#
#
#
